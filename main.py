...
async def suggest_optimizations(ctx):
    try:
        ...
        if not owner or not repo:
            await ctx.send("Invalid GitHub link. Please provide a valid GitHub repository link.")
            return
...
async def review_repository(ctx):
    try:
        ...
        args = ctx.message.content.split(' ')
        if len(args) < 2:
            await ctx.send("Please provide a GitHub link for review.")
            return
        ...
        if not owner or not repo:
            ...
            if not github_link_regex.match(args[1]):
                await ctx.channel.send("Invalid GitHub link. Please provide a valid GitHub repository link.")
